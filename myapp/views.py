...
class Index(TemplateView):
    template_name = 'myapp/index.html' #returnで渡す先？

    def get_context_data(self, *args, **kwargs): #htmlにpostを使えるようにする
        context = super().get_context_data(**kwargs)
        post_list = Post.objects.all().order_by('-created_at') #新しい順
        context = {
            'post_list': post_list,
        }
        return context
...
class PostCreate(LoginRequiredMixin, CreateView):
    model = Post
    form_class = PostForm
    success_url = reverse_lazy('myapp:index') #登録成功したら移動するページの設定

    def form_valid(self,form):
        form.instance.author_id = self.request.user.id
        return super(PostCreate, self).form_valid(form) #PostCreateの中身を上書き
...
class CategoryDetail(DetailView):
    ...
    def get_context_data(self, *args, **kwargs): #ポストの値を取得するため
        detail_data = Category.objects.get(name_en = self.kwargs['name_en']) #情報一つ
        category_posts = Post.objects.filter(category = detail_data.id).order_by('-created_at') #情報複数の時
...
def Search(request):
    if request.method == 'POST':
        ...
        if searchform.is_valid():
            ...
            search_list = Post.objects.filter(Q(title__icontains = freeword)|Q(content__icontains = freeword)) #or検索
